...
output_dir = os.path.abspath(os.path.join(current_dir, '../my-app/src/Components/Common'))
...
# Helper function to clean text and remove non-ASCII characters
def clean_text(text):
...
def process_data(captions):
    ...
    for i, sentiment in enumerate(sentiments):
        pdf.cell(200, 10, txt=f"Caption {i+1}: {clean_text(sentiment)}", ln=True)
    pdf.ln(10)
    ...
    for word, freq in most_common_words:
        pdf.cell(200, 10, txt=f"{clean_text(word)}: {freq}", ln=True)
    pdf.ln(10)
...
def process_file(file_path):
    try:
        ...
        captions = [result.get("snippet", "").strip() for result in data.get("results", []) if result.get("snippet")]

        print(f"Number of valid snippets read: {len(captions)}")
        print("First snippet:", captions[0] if captions else "No valid snippets found")

        if captions:
            process_data(captions)
        else:
            print("No valid snippets found to process.")
